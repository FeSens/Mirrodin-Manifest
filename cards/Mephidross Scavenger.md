---
tags:
  - card
type: Creature
subtype: Zombie
color: Black
mana_cost: "{1}{B}"
cmc: 2
rarity: Common
power: 2
toughness: 1
set: Mirrodin Manifest
---

# Mephidross Scavenger

## Card Type Line
Creature — Zombie

## Rules Text
> When Mephidross Scavenger enters the battlefield, each player mills two cards.

## Flavor Text
> *"The Dross feeds on what the living discard."*

## Image Prompt
> Mephidross Scavenger, Zombie from Mirrodin Manifest. Setting: Dross Dominion — the toxic swamps of Mephidross at night. Mood: Exhaustion. Pose: Crawling on all fours through the muck, clawed hands sifting through debris. Key visual elements: Exposed chrome bones visible through rotting flesh, necrogen-green glow in hollow eye sockets, corroded metal fragments embedded in skin. Style reference: Nils Hamm meets Daarken. Color palette: Near-black, dark olive, sickly necrogen glow.

## Design Notes
- Black common creature - addresses Black common gap (-6)
- Mill synergy for self-mill or opponent disruption
- NWO-compliant simple design
- Lore connection: [[Lore#The Dross (Mephidross)]]
- Pairs with: [[Mephidross Refugee]], [[The Black Ledger]]

## Outside of this set interactions
- **Notable Synergies:** [[Altar of the Brood]] mill triggers when creatures enter; [[Undead Alchemist]] mill creates Zombie tokens; [[Syr Konrad, the Grim]] triggers on mill and creature movement
- **Potential Combos:** [[Mesmeric Orb]] additional mill synergy; [[Bruvac the Grandiloquent]] doubles mill; [[Phenax, God of Deception]] tap creatures to mill
- **Balance Assessment:** Fair - Mill 2 for each player is minor. Helps graveyard strategies but not broken
